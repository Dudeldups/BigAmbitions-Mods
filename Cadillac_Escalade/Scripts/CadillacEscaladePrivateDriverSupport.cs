...
internal static class CadillacEscaladePrivateDriverSupport
{
    ...
    internal static ModContext? Context { get; set; }
    ...
    private static GameObject? CreateAiPrefab(GameObject playerPrefab)
    {
        ...
        // The seat reference belongs to CarHolder's animation hierarchy, not
        // CadillacVisual. The AI prefab copies only the visual roots above.
        var playerSeat = FindTransform(playerPrefab.transform, "Animate_SteeringWheel_033");
        if (playerSeat != null)
        {
            var aiSeat = new GameObject(playerSeat.name);
        ...
        {
            Debug.LogWarning("CadillacEscalade NPC driver seat source is missing.");
        }
        ...
        clone.AddComponent<CadillacEscaladeAmbientTrafficLighting>();
        clone.AddComponent<CadillacEscaladeAmbientTrafficDriver>();
...
internal sealed class CadillacEscaladeAmbientTrafficLighting : MonoBehaviour
{
    private void Start()
    {
        var lighting = GetComponent<CadillacEscaladeLightingController>() ??
                       gameObject.AddComponent<CadillacEscaladeLightingController>();
        lighting.InitializeForAmbientTraffic(CadillacEscaladePrivateDriverSupport.Context);
    }
}

internal sealed class CadillacEscaladeAmbientTrafficDriver : MonoBehaviour
{
    private void Start()
    {
        var driver = GetComponent<CadillacEscaladeDriverController>() ??
                     gameObject.AddComponent<CadillacEscaladeDriverController>();
        driver.InitializeForAmbientTraffic(CadillacEscaladePrivateDriverSupport.Context);
    }
}
