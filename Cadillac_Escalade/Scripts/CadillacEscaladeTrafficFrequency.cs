...
[AddComponentMenu("")]
internal sealed class CadillacEscaladeTrafficFrequency : MonoBehaviour
{
    ...
    private void HandleVehicleAdded(int vehicleIndex)
    {
        if (TryGetIdleVehicles(out _, out var trafficVehicles))
        {
            ...
            {
                totalSpawns++;
                var prefab = CadillacEscaladePrivateDriverSupport.TrafficPrefab;
    ...
    {
        if (totalSpawns == 0 || !CadillacEscaladeDiagnostics.DebugEnabled ||
            !CadillacEscaladeDiagnostics.NpcTrafficDebugEnabled)
            return;
        context?.Logger.Info(
            "CadillacEscalade traffic frequency: " +
        ...
        out List<VehicleComponent>? idle, out TrafficVehicles trafficVehicles)
    {
        ...
        if (!missingFieldReported)
        {
            ...
            context?.Logger.Warn(
                "CadillacEscalade traffic frequency: idle list unavailable; NPC frequency unchanged.");
        }
        return false;
    }
    ...
    private void RebalanceIdleVehicles()
    {
        ...
        if (!firstBalanceLogged && CadillacEscaladeDiagnostics.DebugEnabled &&
            CadillacEscaladeDiagnostics.NpcTrafficDebugEnabled)
        {
