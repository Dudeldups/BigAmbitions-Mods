...
internal sealed class CadillacEscaladeDriverController : MonoBehaviour
{
    ...
    private void LateUpdate()
    {
        ...
        var isOccupied = ambientTraffic
            ? GetComponent<PrivateDriverVehicle>() == null
            : vehicle != null && vehicle.controlledByPlayer;
        if (isOccupied != occupied)
        {
            ...
            {
                LogInfo(ambientTraffic
                    ? "NPC spawned; preparing seated driver appearance."
                    : "occupied; preparing current player appearance.");
            }
        }
        ...
        catch (Exception ex)
        {
            ...
            {
                lastFailure = reason;
                context?.Logger.Warn($"CadillacEscalade driver vehicle={vehicleRoot.GetInstanceID()} " +
    ...
    private void CreateDriver()
    {
        ...
        if (ambientTraffic)
        {
            // Traffic is independent of the player's current clothing/render state.
            ...
            sourceNpc.name = "CadillacEscalade_NpcAppearanceSource";
        ...
        driverRoot = new GameObject(ambientTraffic
            ? "CadillacEscalade_SeatedNpcDriver" : "CadillacEscalade_SeatedPlayer");
        ...
        if (rendererCount == 0)
            throw new InvalidOperationException(
                ...
                $"total={appearance.GetComponentsInChildren<SkinnedMeshRenderer>(true).Length} " +
                $"suppressed={suppressedCount} ambient={ambientTraffic}.");
        ...
        LogInfo($"created {(ambientTraffic ? "native pedestrian" : "player")} driver; renderers={rendererCount} " +
    ...
    private void AlignHand(SeatedLimb? arm, float targetX)
    {
        if (arm == null || vehicleRoot == null)
            return;
        var target = vehicleRoot.InverseTransformPoint(arm.End.position);
        target.x = targetX;
    ...
    {
        if (ambientTraffic && !CadillacEscaladeDiagnostics.NpcDriverDebugEnabled)
            return;
        CadillacEscaladeDiagnostics.Info(
            context,
            $"CadillacEscalade driver vehicle={vehicleRoot?.GetInstanceID()} " +
            $"ambient={ambientTraffic}: {message}");
    }
