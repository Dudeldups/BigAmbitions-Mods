...
[AddComponentMenu("")]
internal sealed class DodgeChallenger2018TrafficFrequency : MonoBehaviour
{
    ...
    private void HandleVehicleAdded(int vehicleIndex)
    {
        if (TryGetIdleVehicles(out _, out var trafficVehicles))
        {
            ...
            {
                totalSpawns++;
                var prefab = DodgeChallenger2018PrivateDriverSupport.TrafficPrefab;
    ...
    {
        if (totalSpawns == 0 || !DodgeChallenger2018Diagnostics.DebugEnabled ||
            !DodgeChallenger2018Diagnostics.NpcTrafficDebugEnabled)
            return;
        context?.Logger.Info(
            "DodgeChallenger2018 traffic frequency: " +
        ...
        out List<VehicleComponent>? idle, out TrafficVehicles trafficVehicles)
    {
        ...
        if (!missingFieldReported)
        {
            ...
            context?.Logger.Warn(
                "DodgeChallenger2018 traffic frequency: idle list unavailable; NPC frequency unchanged.");
        }
        return false;
    }
    ...
    private void RebalanceIdleVehicles()
    {
        ...
        if (!firstBalanceLogged && DodgeChallenger2018Diagnostics.DebugEnabled &&
            DodgeChallenger2018Diagnostics.NpcTrafficDebugEnabled)
        {
