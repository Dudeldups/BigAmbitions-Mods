...
internal static class DodgeChallenger2018LoadRecovery
{
    ...
    internal static bool CompleteInterruptedLoad(ModContext? context)
    {
        ...
        foreach (var vehicle in save.privateDriverVehicleInstances)
            hasDodge |= vehicle != null && string.Equals(vehicle.vehicleTypeName,
                DodgeChallenger2018Mod.VehicleTypeName, StringComparison.Ordinal);
        if (!hasDodge)
            return false;

        attemptedManager = manager;
        var exitRegistered = HasNativeExitCallback(manager);
        DodgeChallenger2018Diagnostics.LoadRecoveryInfo(context, $"DodgeChallenger2018: chauffeur save startup check: " +
        ...
        {
            context.Logger.Warn("DodgeChallenger2018: startup recovery skipped: native initialization state differs from the supported interrupted-Awake path.");
            return false;
        }
        ...
        foreach (var vehicle in save.VehicleInstances)
        {
            ...
            {
                context.Logger.Warn($"DodgeChallenger2018: startup recovery stopped: player vehicle type still unresolved: '{vehicle?.vehicleTypeName ?? "<null record>"}'.");
                return false;
            }
        }
        foreach (var vehicle in save.privateDriverVehicleInstances)
        {
            if (vehicle == null || vehicle.VehicleType == null)
            {
                context.Logger.Warn($"DodgeChallenger2018: startup recovery stopped: chauffeur vehicle type still unresolved: '{vehicle?.vehicleTypeName ?? "<null record>"}'.");
                return false;
            }
        }
        ...
        try
        {
            ...
            context.Logger.Warn($"DodgeChallenger2018: recovered interrupted city startup after vehicle registration; " +
        ...
        catch (Exception exception)
        {
            ...
            context.Logger.Warn($"DodgeChallenger2018: interrupted-load recovery failed: {failure}");
            return false;
        }
    }
    ...
    internal static void ReportInputState(ModContext? context)
    {
        ...
        if (!recoveredManager.isActiveAndEnabled || !InputHelper.IsInitialized())
            context.Logger.Warn($"DodgeChallenger2018: recovered city is not ready for input: " +
