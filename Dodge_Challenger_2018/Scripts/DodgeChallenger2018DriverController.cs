...
internal sealed class DodgeChallenger2018DriverController : MonoBehaviour
{
    private const string SteeringWheelMarker = "DodgeSteeringReference";
    private const string SittingClipName = "SitDeliveryTruck";
    private const float SeatedScale = 0.94f;
    private const float SeatBackLeanDegrees = 23f;
    private const float HandHalfSpacing = 0.19f;
    private const float HandForwardOffset = -0.065f;
    private const float HandHeightOffset = 0f;
    private const float FootRaise = 0.18f;
    private const float FootForwardOffset = 0.26f;
    // Pelvis position relative to the Challenger steering-wheel pivot, in vehicle axes.
    private static readonly Vector3 SeatOffset = new(0f, -0.36f, -0.60f);
    ...
    private void LateUpdate()
    {
        ...
        catch (Exception ex)
        {
            ...
            {
                lastFailure = reason;
                context?.Logger.Warn($"DodgeChallenger2018 driver vehicle={vehicleRoot.GetInstanceID()} " +
    ...
    private void CreateDriver()
    {
        ...
        if (ambientTraffic)
        {
            ...
            sourceNpc.name = "DodgeChallenger2018_NpcAppearanceSource";
        ...
        if (steeringWheel == null)
            throw new InvalidOperationException("Challenger steering-wheel seat reference is missing.");
        ...
        driverRoot = new GameObject(ambientTraffic ? "DodgeChallenger2018_SeatedNpcDriver" : "DodgeChallenger2018_SeatedPlayer");
        ...
        poseGraph = PlayableGraph.Create("DodgeChallenger2018 seated player");
        ...
        if (ambientTraffic && DodgeChallenger2018Diagnostics.DebugEnabled &&
            DodgeChallenger2018Diagnostics.NpcDriverDebugEnabled)
            context?.Logger.Info($"DodgeChallenger2018 NPC driver vehicle={vehicleRoot?.GetInstanceID()} " +
    ...
    private string VehiclePosition(Transform? target) =>
        target != null && vehicleRoot != null ? vehicleRoot.InverseTransformPoint(target.position).ToString("F3") : "missing";

    private void LogInfo(string message)
    {
        if (ambientTraffic && !DodgeChallenger2018Diagnostics.NpcDriverDebugEnabled)
            return;
        DodgeChallenger2018Diagnostics.Info(
            context,
            $"DodgeChallenger2018 driver vehicle={vehicleRoot?.GetInstanceID()} ambient={ambientTraffic}: {message}");
    }
