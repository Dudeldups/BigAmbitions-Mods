...
/// reached before the vehicle-layer trigger can receive the Dodge collider.
/// </summary>
[AddComponentMenu("")]
[DisallowMultipleComponent]
internal sealed class DodgeChallenger2018WarehouseEntryController : MonoBehaviour
{
    ...
    private void EnsureApproachTrigger()
    {
        ...
        var triggerObject = new GameObject("DodgeChallenger2018_WarehouseApproachTrigger")
        {
    ...
    internal void SuppressEntrance(DriveInEntrance entrance, string reason)
    {
        ...
        DodgeChallenger2018Diagnostics.WarehouseExitInfo(
            context,
            $"DodgeChallenger2018 warehouse-entry: suppressing physical-door fallback for " +
    ...
    internal void ClearSuppressedEntrance(DriveInEntrance? entrance, string reason)
    {
        ...
        DodgeChallenger2018Diagnostics.WarehouseExitInfo(
            context,
            $"DodgeChallenger2018 warehouse-entry: restoring physical-door fallback for " +
    ...
    private void OnCollisionEnter(Collision collision)
    {
        ...
        DodgeChallenger2018Diagnostics.WarehouseExitInfo(
            context,
            $"DodgeChallenger2018 warehouse-entry: physical door collision invokes native entry; " +
    ...
    private void OnTriggerEnter(Collider other)
    {
        ...
        if (entrance == suppressedEntrance)
        {
            if (Time.unscaledTime >= nextSuppressionLogTime)
            {
                ...
                DodgeChallenger2018Diagnostics.WarehouseExitInfo(
                    context,
                    $"DodgeChallenger2018 warehouse-entry: approach trigger suppressed during warehouse " +
        ...
        DodgeChallenger2018Diagnostics.WarehouseExitInfo(
            context,
            $"DodgeChallenger2018 warehouse-entry: approach trigger invokes native entry; " +
            $"instance={vehicle.GetInstanceID()}, entrance='{entrance.name}', " +
        ...
        Vector3 relativeVelocity)
    {
        ...
        DodgeChallenger2018Diagnostics.WarehouseExitInfo(
            context,
            $"DodgeChallenger2018 warehouse-entry: physical-door fallback suppressed during " +
