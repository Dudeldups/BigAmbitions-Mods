...
internal sealed class DodgeChallenger2018LightingController : MonoBehaviour
{
    ...
    private void InitializeVisuals(GameObject root)
    {
        ...
        var coolWhite = new Color(0.88f, 0.94f, 1f, 1f);
        var amber = new Color(1f, 0.34f, 0.015f, 1f);
        var rearAmber = new Color(1f, 0.68f, 0.010f, 1f);
        var red = new Color(1f, 0.006f, 0.002f, 1f);

        // Lamp geometry is cut into persistent mesh assets by the Unity setup.
        // Runtime glTF renderers can survive AssetBundle loading without their
        // original readable MeshFilter, which made runtime triangle extraction fail.
        headlampOverlay = PreparePersistentOverlay(
            renderers, "DodgeLamp_FrontWhiteLeft", coolWhite, 5.6f);
        headlampOverlayRight = PreparePersistentOverlay(
            renderers, "DodgeLamp_FrontWhiteRight", coolWhite, 5.6f);
        leftBlinkerOverlay = PreparePersistentOverlay(
            renderers, "DodgeLamp_FrontIndicatorLeft", amber, 5.5f);
        rightBlinkerOverlay = PreparePersistentOverlay(
            renderers, "DodgeLamp_FrontIndicatorRight", amber, 5.5f);
        rearTailOverlay = PreparePersistentOverlay(
            renderers, "DodgeLamp_RearTail", red, 1.8f);
        rearBrakeOverlay = PreparePersistentOverlay(
            renderers, "DodgeLamp_RearBrake", red, 4.2f);
        thirdBrakeOverlay = PreparePersistentOverlay(
            renderers, "DodgeLamp_ThirdBrake", red, 5.2f);
        reverseOverlay = PreparePersistentOverlay(
            renderers, "DodgeLamp_Reverse", coolWhite, 4.8f);
        rearLeftBlinkerOverlay = PreparePersistentOverlay(
            renderers, "DodgeLamp_RearIndicatorLeft", rearAmber, 5.8f);
        rearRightBlinkerOverlay = PreparePersistentOverlay(
            renderers, "DodgeLamp_RearIndicatorRight", rearAmber, 5.8f);
        PrepareRearHousing(renderers, "DodgeLamp_RearHousingLeft");
        PrepareRearHousing(renderers, "DodgeLamp_RearHousingRight");
        daylightOverlay = null;
        daylightOverlayRight = null;
        rearBrakeSegmentsOverlay = null;

        var persistentCount = CountLampOverlays() + CountBlinkerOverlays();
        if (persistentCount < 8)
            LogWarning($"persistent lamp setup incomplete overlays={persistentCount}; rebuild the AssetBundle.");

        var beamCount = trafficLights == null ? ConfigureHeadlightBeams() : 0;
        initialized = true;
        LogInfo($"initialized persistentLampMeshes=true npc={trafficLights != null} beams={beamCount}/2 " +
                $"lampOverlays={CountLampOverlays()} blinkerOverlays={CountBlinkerOverlays()}.");
        if (trafficLights == null && blinkers == null)
    ...
    private int ConfigureHeadlightBeams()
    {
        ...
        {
            LogWarning("inherited Spotlights road-light template is missing.");
            return 0;
        }
        templateBeam.enabled = false;
        leftBeam = CloneBeam(-0.58f, "LeftHeadlightBeam");
        rightBeam = CloneBeam(0.58f, "RightHeadlightBeam");
    ...
    private Light? CloneBeam(float localX, string suffix)
    {
        ...
        clone.name = "DodgeChallenger2018_" + suffix;
        clone.transform.localPosition = new Vector3(localX, 0.72f, 2.08f);
        clone.transform.localRotation = Quaternion.Euler(5f, 0f, 0f);
        var light = clone.GetComponent<Light>();
        if (light == null) { Destroy(clone); return null; }
        light.enabled = false;
        light.cookie = null;
        light.range = 52f;
        light.spotAngle = 60f;
    ...
    private void PrepareRearHousing(IEnumerable<MeshRenderer> renderers, string objectName)
    {
        foreach (var renderer in renderers)
        {
            ...
            var housing = new Color(0.075f, 0.0025f, 0.002f, 1f);
            SetColor(material, "_UnlitColor", housing);
            SetColor(material, "_BaseColor", housing);
            SetColor(material, "_Color", housing);
            SetColor(material, "baseColorFactor", housing);
            ...
            SetFloat(material, "_CullMode", (float)UnityEngine.Rendering.CullMode.Off);
        ...
        LogWarning($"rear lamp housing '{objectName}' was not found.");
    }

    private MeshRenderer? PreparePersistentOverlay(
        ...
        float intensity)
    {
        foreach (var renderer in renderers)
        {
            ...
            {
                LogWarning($"persistent overlay '{objectName}' has no mesh.");
                return null;
            }

            renderer.sharedMaterial = CreateUnlitMaterial(objectName + " Material", color, intensity);
            renderer.shadowCastingMode = ShadowCastingMode.Off;
            renderer.receiveShadows = false;
        ...
        LogWarning($"persistent overlay '{objectName}' was not found.");
        return null;
    }
    ...
    {
        var host = new GameObject("DodgeChallenger2018_" + suffix);
    ...
    private void ApplyState()
    {
        ...
        var flash = npc
            ? blinking && TrafficBlinkerPhase?.GetValue(trafficLights) is int phase && phase != 0
            : blinking && Mathf.Repeat(Time.unscaledTime - blinkerPhaseStartedAt,
                BlinkerHalfPeriod * 2f) < BlinkerHalfPeriod;
        wasBlinking = blinking;

        SetEnabled(daylightOverlay, npc || lightsOn);
        SetEnabled(daylightOverlayRight, npc || lightsOn);
        ...
        if (state == lastState)
            return;
        lastState = state;
        LogInfo($"state npc={npc} playerControlled={controlled} lights={lightsOn} braking={braking} " +
    ...
    private static MeshRenderer? FindRendererByNameOrMaterial(
        IEnumerable<MeshRenderer> renderers,
        string rendererMarker,
        string materialMarker)
    {
        MeshRenderer? materialFallback = null;
        foreach (var renderer in renderers)
        {
            if (renderer == null)
                continue;

            if (renderer.name.IndexOf(rendererMarker, StringComparison.OrdinalIgnoreCase) >= 0)
                return renderer;

            foreach (var material in renderer.sharedMaterials)
            {
                if (material == null ||
                    material.name.IndexOf(materialMarker, StringComparison.OrdinalIgnoreCase) < 0)
                    continue;
                materialFallback ??= renderer;
                break;
            }
        }
        ...
        string materialMarker)
    {
        foreach (var renderer in renderers)
        {
            if (renderer == null)
                continue;
            if (!string.IsNullOrEmpty(hierarchyMarker) &&
                !HasAncestor(renderer.transform, hierarchyMarker))
                continue;
            foreach (var material in renderer.sharedMaterials)
    ...
    private void LogInfo(string message) =>
        DodgeChallenger2018Diagnostics.Info(
            context,
            $"DodgeChallenger2018 lighting vehicle='{vehicleRoot?.name}' " +
            $"instance={vehicleRoot?.GetInstanceID()}: {message}");

    private void LogWarning(string message) =>
        context?.Logger.Warn($"DodgeChallenger2018 lighting vehicle='{vehicleRoot?.name}' " +
                             $"instance={vehicleRoot?.GetInstanceID()}: {message}");
