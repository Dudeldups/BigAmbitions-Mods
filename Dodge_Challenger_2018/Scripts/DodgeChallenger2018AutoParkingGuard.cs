...
internal sealed class DodgeChallenger2018AutoParkingGuard : MonoBehaviour
{
    ...
    internal static DodgeChallenger2018AutoParkingGuard Install(
        GameObject owner, ModContext context, string vehicleTypeName)
    {
        var guard = owner.GetComponent<DodgeChallenger2018AutoParkingGuard>() ??
                    owner.AddComponent<DodgeChallenger2018AutoParkingGuard>();
        guard.context = context;
        guard.vehicleTypeName = vehicleTypeName;
        if (LastAutoParkPositionField == null || HudParkingStateField == null)
            DodgeChallenger2018Diagnostics.AutoParkingWarn(context,
                "DodgeChallenger2018 auto-parking: required game parking field is unavailable.");
        var marker = Path.Combine(Application.persistentDataPath,
            "ModsLocal", "Dodge_Challenger_2018", "Config", "auto-parking-diagnostics.enabled");
        if (File.Exists(marker))
        {
            DodgeChallenger2018Diagnostics.DebugEnabled = true;
            DodgeChallenger2018Diagnostics.AutoParkingDebugEnabled = true;
            context.Logger.Info("DodgeChallenger2018 auto-parking diagnostics enabled.");
        }
    ...
    private bool DiagnosticsEnabled =>
        ...
        DodgeChallenger2018Diagnostics.AutoParkingDebugEnabled;
    ...
    private void LateUpdate()
    {
        ...
        if (selectedVehicle != vehicle)
        {
            ...
            if (parkingHelper == null)
                DodgeChallenger2018Diagnostics.AutoParkingWarn(context,
                    $"DodgeChallenger2018 auto-parking: vehicle={vehicle.GetInstanceID()} " +
        ...
        if (DiagnosticsEnabled &&
            ...
            DodgeChallenger2018Diagnostics.AutoParkingInfo(context,
                $"DodgeChallenger2018 auto-parking refresh vehicle={vehicle.GetInstanceID()} " +
    ...
    private IEnumerator ReconcileAfterExit(VehicleController vehicle)
    {
        ...
        if (DiagnosticsEnabled)
            DodgeChallenger2018Diagnostics.AutoParkingInfo(context,
                $"DodgeChallenger2018 auto-parking exit vehicle={vehicle.GetInstanceID()} " +
