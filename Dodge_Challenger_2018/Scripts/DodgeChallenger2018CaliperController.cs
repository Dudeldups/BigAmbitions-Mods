...
internal sealed class DodgeChallenger2018CaliperController : MonoBehaviour
{
    private static readonly string[,] BindingNames =
    {
        { "DodgeFixedCaliperFrontLeft", "DodgeWheelFrontLeft" },
        { "DodgeFixedCaliperFrontRight", "DodgeWheelFrontRight" },
        { "DodgeFixedCaliperRearLeft", "DodgeWheelRearLeft" },
        { "DodgeFixedCaliperRearRight", "DodgeWheelRearRight" },
    };
    ...
    public void Initialize(VehicleController controller, ModContext? modContext)
    {
        ...
        DodgeChallenger2018Diagnostics.Info(
            context,
            $"DodgeChallenger2018 steering calipers ready vehicle={controller.GetInstanceID()}, " +
        ...
        foreach (var binding in bindings)
        {
            ...
            if (Mathf.Abs(steeringAngle) > 60f)
            {
                if (!warnedAboutInvalidSteering)
                {
                    ...
                    context?.Logger.Warn(
                        $"DodgeChallenger2018 steering caliper rejected angle={steeringAngle:0.0}deg " +
