...
[AddComponentMenu("")]
internal sealed class AudiRS6RTrafficFrequency : MonoBehaviour
{
    private const int TargetSpawnsPerCar = 50;
    ...
    private void HandleVehicleAdded(int vehicleIndex)
    {
        if (TryGetIdleVehicles(out _, out var trafficVehicles))
        {
            ...
            {
                totalSpawns++;
                var prefab = AudiRS6RPrivateDriverSupport.TrafficPrefab;
    ...
    {
        if (totalSpawns == 0 || !AudiRS6RDiagnostics.DebugEnabled ||
            !AudiRS6RDiagnostics.NpcTrafficDebugEnabled)
            return;
        context?.Logger.Info(
            "AudiRS6R traffic frequency: " +
        ...
        out List<VehicleComponent>? idle, out TrafficVehicles trafficVehicles)
    {
        ...
        if (!missingFieldReported)
        {
            ...
            context?.Logger.Warn(
                "AudiRS6R traffic frequency: idle list unavailable; NPC frequency unchanged.");
        }
        return false;
    }
    ...
    private void RebalanceIdleVehicles()
    {
        ...
        if (!firstBalanceLogged && AudiRS6RDiagnostics.DebugEnabled &&
            AudiRS6RDiagnostics.NpcTrafficDebugEnabled)
        {
