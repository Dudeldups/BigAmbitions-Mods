...
internal sealed class AudiRS6RAutoParkingGuard : MonoBehaviour
{
    ...
    internal static AudiRS6RAutoParkingGuard Install(
        GameObject owner, ModContext context, string vehicleTypeName)
    {
        var guard = owner.GetComponent<AudiRS6RAutoParkingGuard>() ??
                    owner.AddComponent<AudiRS6RAutoParkingGuard>();
        guard.context = context;
        guard.vehicleTypeName = vehicleTypeName;
        if (LastAutoParkPositionField == null || HudParkingStateField == null)
            AudiRS6RDiagnostics.AutoParkingWarn(context,
                "AudiRS6R auto-parking: required game parking field is unavailable.");
        var marker = Path.Combine(Application.persistentDataPath,
            "ModsLocal", "AudiRS6R", "Config", "auto-parking-diagnostics.enabled");
        if (File.Exists(marker))
        {
            AudiRS6RDiagnostics.DebugEnabled = true;
            AudiRS6RDiagnostics.AutoParkingDebugEnabled = true;
            context.Logger.Info("AudiRS6R auto-parking diagnostics enabled.");
        }
    ...
    private bool DiagnosticsEnabled =>
        AudiRS6RDiagnostics.DebugEnabled &&
        AudiRS6RDiagnostics.AutoParkingDebugEnabled;
    ...
    private void LateUpdate()
    {
        ...
        if (selectedVehicle != vehicle)
        {
            ...
            if (parkingHelper == null)
                AudiRS6RDiagnostics.AutoParkingWarn(context,
                    $"AudiRS6R auto-parking: vehicle={vehicle.GetInstanceID()} " +
        ...
        if (DiagnosticsEnabled &&
            ...
            AudiRS6RDiagnostics.AutoParkingInfo(context,
                $"AudiRS6R auto-parking refresh vehicle={vehicle.GetInstanceID()} " +
    ...
    private IEnumerator ReconcileAfterExit(VehicleController vehicle)
    {
        ...
        if (DiagnosticsEnabled)
            AudiRS6RDiagnostics.AutoParkingInfo(context,
                $"AudiRS6R auto-parking exit vehicle={vehicle.GetInstanceID()} " +
