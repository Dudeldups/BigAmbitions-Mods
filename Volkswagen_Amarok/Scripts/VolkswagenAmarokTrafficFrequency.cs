...
[AddComponentMenu("")]
internal sealed class VolkswagenAmarokTrafficFrequency : MonoBehaviour
{
    private const int TargetSpawnsPerHundred = 3;
    ...
    private void HandleVehicleAdded(int vehicleIndex)
    {
        if (TryGetIdleVehicles(out _, out var trafficVehicles))
        {
            ...
            {
                totalSpawns++;
                var prefab = VolkswagenAmarokPrivateDriverSupport.TrafficPrefab;
    ...
    {
        if (totalSpawns == 0 || !VolkswagenAmarokDiagnostics.DebugEnabled ||
            !VolkswagenAmarokDiagnostics.NpcTrafficDebugEnabled)
            return;
        context?.Logger.Info(
            "VolkswagenAmarok traffic frequency: " +
        ...
        out List<VehicleComponent>? idle, out TrafficVehicles trafficVehicles)
    {
        ...
        if (!missingFieldReported)
        {
            ...
            context?.Logger.Warn(
                "VolkswagenAmarok traffic frequency: idle list unavailable; NPC frequency unchanged.");
        }
        return false;
    }
    ...
    private void RebalanceIdleVehicles()
    {
        ...
        var due = (long)ownSpawns * 100 <= (long)totalSpawns * TargetSpawnsPerHundred;
        if (due)
        {
        ...
        if (!firstBalanceLogged && VolkswagenAmarokDiagnostics.DebugEnabled &&
            VolkswagenAmarokDiagnostics.NpcTrafficDebugEnabled)
        {
            ...
            context?.Logger.Info(
                ...
                $"own={ownSpawns} total={totalSpawns} target={TargetSpawnsPerHundred}%.");
        }
    }
}
