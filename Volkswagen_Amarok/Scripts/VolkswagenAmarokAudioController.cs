...
internal sealed class VolkswagenAmarokAudioController : MonoBehaviour
{
    ...
    private bool TryConfigure()
    {
        ...
        audioHost = new GameObject("VolkswagenAmarok_EngineLayers");
        ...
        crackleClip = VolkswagenAmarokCrackleWave.Create();
        var exhaustHost = new GameObject("VolkswagenAmarok_ExhaustCrackle");
        ...
        var hornHost = new GameObject("VolkswagenAmarok_Horn");
        ...
        VolkswagenAmarokDiagnostics.Info(
            context,
            $"VolkswagenAmarok audio configured vehicle={vehicle.GetInstanceID()}, " +
            $"engineLayers=7, engineGain={VolkswagenAmarokAudioModel.EngineBaseVolume:0.00}.." +
            $"{VolkswagenAmarokAudioModel.EngineBaseVolume + VolkswagenAmarokAudioModel.EngineThrottleVolume:0.00}, " +
            $"hornVoices=low/high@{VolkswagenAmarokAudioModel.HornLowVolume:0.00}/" +
            $"{VolkswagenAmarokAudioModel.HornHighVolume:0.00}, " +
    ...
    {
        var clip = VolkswagenAmarokWave.Load(Path.Combine(context!.ModRootPath, "Config", "Audio", name + ".wav"));
        ownedClips.Add(clip);
        return clip;
    }
    ...
    private void UpdatePlayback()
    {
        ...
        if (!paused)
        {
            ...
            var normalized = VolkswagenAmarokAudioModel.Normalize(smoothRpm, engine.idleRPM, engine.revLimiterRPM);
            ...
            driveBlend = Mathf.MoveTowards(driveBlend,
                VolkswagenAmarokAudioModel.DrivingBlend(rawRpm, engine.idleRPM, engine.revLimiterRPM), Time.deltaTime * 4f);
            idleSource.pitch = VolkswagenAmarokAudioModel.IdlePitch;
            idleSource.volume = envelope * master * mixGain *
                                VolkswagenAmarokAudioModel.IdleVolume(driveBlend);
            idleSource.mute = controlled && savedMute;
            var gain = envelope * master * mixGain * VolkswagenAmarokAudioModel.EngineVolume(smoothThrottle) *
                       Mathf.Sqrt(driveBlend);
            loadBlend = VolkswagenAmarokAudioModel.LoadBlend(smoothThrottle);
            for (var i = 0; i < EngineNames.Length; i++)
            {
                layers[i].pitch = VolkswagenAmarokAudioModel.Pitch(normalized,i);
                layers[i + 3].pitch = layers[i].pitch;
                var bandGain = gain * VolkswagenAmarokAudioModel.Weight(normalized, i);
            ...
            crackleSource.volume = envelope * master * mixGain * Mathf.Lerp(
                VolkswagenAmarokAudioModel.CrackleIdleVolume,
                VolkswagenAmarokAudioModel.CrackleLoadVolume,
                crackleLoad);
    ...
    private void UpdateHorn(bool pressed, float master)
    {
        ...
        var lowTarget = pressed ? master * VolkswagenAmarokAudioModel.HornLowVolume : 0f;
        var highTarget = pressed ? master * VolkswagenAmarokAudioModel.HornHighVolume : 0f;
    ...
    private void Warn(string message) => context?.Logger.Warn($"VolkswagenAmarok audio vehicle={vehicle?.GetInstanceID()}: {message}");
