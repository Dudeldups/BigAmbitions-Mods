...
internal sealed class VolkswagenAmarokCaliperController : MonoBehaviour
{
    private static readonly string[,] BindingNames =
    {
        { "AmarokFixedCaliperFrontLeft", "AmarokWheelFrontLeft" },
        { "AmarokFixedCaliperFrontRight", "AmarokWheelFrontRight" },
        { "AmarokFixedCaliperRearLeft", "AmarokWheelRearLeft" },
        { "AmarokFixedCaliperRearRight", "AmarokWheelRearRight" },
    };
    ...
    public void Initialize(VehicleController controller, ModContext? modContext)
    {
        ...
        VolkswagenAmarokDiagnostics.Info(
            context,
            $"VolkswagenAmarok steering calipers ready vehicle={controller.GetInstanceID()}, " +
        ...
        foreach (var binding in bindings)
        {
            ...
            if (Mathf.Abs(steeringAngle) > 60f)
            {
                if (!warnedAboutInvalidSteering)
                {
                    ...
                    context?.Logger.Warn(
                        $"VolkswagenAmarok steering caliper rejected angle={steeringAngle:0.0}deg " +
