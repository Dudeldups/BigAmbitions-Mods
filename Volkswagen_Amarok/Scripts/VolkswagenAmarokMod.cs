...
[assembly: RegisterModClass(typeof(VolkswagenAmarokMod))]

internal static class VolkswagenAmarokDiagnostics
{
...
public sealed class VolkswagenAmarokMod : IModBigAmbitions
{
    internal const string VehicleTypeName =
        "volkswagenamarok-vehicle:vehicletype_volkswagenamarok";

    private const string BundleKey = "AssetBundles/volkswagenamarok.unity3d";
    private const string VehicleAssetPath =
        "Assets/Mods/Volkswagen_Amarok/VolkswagenAmarok.asset";
    private const string VehiclePrefabPath =
        "Assets/Mods/Volkswagen_Amarok/VolkswagenAmarok.prefab";

    private VehicleType? vehicleType;
    private VolkswagenAmarokRuntime? runtime;
    ...
    public Task OnLoadAsync(ModContext context)
    {
        ...
        if (bundle == null)
        {
            context.Logger.Warn($"VolkswagenAmarok: failed to load bundle '{BundleKey}'.");
        ...
        {
            context.Logger.Warn(
                $"VolkswagenAmarok: failed to load vehicle type '{VehicleAssetPath}'.");
            return Task.CompletedTask;
        }
        ...
        {
            context.Logger.Warn(
                $"VolkswagenAmarok: failed to load vehicle prefab '{VehiclePrefabPath}'.");
            return Task.CompletedTask;
        }
        ...
        VolkswagenAmarokDiagnostics.Info(
            context,
            $"VolkswagenAmarok: registered '{vehicleType.vehicleTypeName}' " +
            $"price={vehicleType.price:0}, maxSpeed={vehicleType.maxSpeed}, " +
            $"enginePower={vehicleType.enginePower:0}.");
        runtime = VolkswagenAmarokRuntime.Initialize(
            context,
    ...
    public Task OnUnloadAsync()
    {
        ...
        if (vehicleType != null)
        {
            VolkswagenAmarokCityCarsDealerStock.RemoveVehicle(vehicleType.vehicleTypeName);
...
internal static class VolkswagenAmarokCityCarsDealerStock
{
