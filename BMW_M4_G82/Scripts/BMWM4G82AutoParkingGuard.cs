...
internal sealed class BMWM4G82AutoParkingGuard : MonoBehaviour
{
    ...
    internal static BMWM4G82AutoParkingGuard Install(
        GameObject owner, ModContext context, string vehicleTypeName)
    {
        var guard = owner.GetComponent<BMWM4G82AutoParkingGuard>() ??
                    owner.AddComponent<BMWM4G82AutoParkingGuard>();
        guard.context = context;
        guard.vehicleTypeName = vehicleTypeName;
        if (LastAutoParkPositionField == null || HudParkingStateField == null)
            BMWM4G82Diagnostics.AutoParkingWarn(context,
                "BMWM4G82 auto-parking: required game parking field is unavailable.");
        var marker = Path.Combine(Application.persistentDataPath,
            "ModsLocal", "BMW_M4_G82", "Config", "auto-parking-diagnostics.enabled");
        if (File.Exists(marker))
        {
            BMWM4G82Diagnostics.DebugEnabled = true;
            BMWM4G82Diagnostics.AutoParkingDebugEnabled = true;
            context.Logger.Info("BMWM4G82 auto-parking diagnostics enabled.");
        }
    ...
    private bool DiagnosticsEnabled =>
        BMWM4G82Diagnostics.DebugEnabled &&
        BMWM4G82Diagnostics.AutoParkingDebugEnabled;
    ...
    private void LateUpdate()
    {
        ...
        if (selectedVehicle != vehicle)
        {
            ...
            if (parkingHelper == null)
                BMWM4G82Diagnostics.AutoParkingWarn(context,
                    $"BMWM4G82 auto-parking: vehicle={vehicle.GetInstanceID()} " +
        ...
        if (DiagnosticsEnabled &&
            ...
            BMWM4G82Diagnostics.AutoParkingInfo(context,
                $"BMWM4G82 auto-parking refresh vehicle={vehicle.GetInstanceID()} " +
    ...
    private IEnumerator ReconcileAfterExit(VehicleController vehicle)
    {
        ...
        if (DiagnosticsEnabled)
            BMWM4G82Diagnostics.AutoParkingInfo(context,
                $"BMWM4G82 auto-parking exit vehicle={vehicle.GetInstanceID()} " +
