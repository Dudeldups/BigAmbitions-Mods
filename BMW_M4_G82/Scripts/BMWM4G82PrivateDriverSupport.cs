...
internal static class BMWM4G82PrivateDriverSupport
{
    ...
    private static GameObject? CreateAiPrefab(GameObject playerPrefab)
    {
        ...
        foreach (var renderer in templateRenderers)
            renderer.enabled = false;
        // Keep a seat marker on the AI prefab, which only copies selected visual roots.
        ...
        if (playerSeat != null)
        {
            ...
            Debug.LogWarning("BMWM4G82 NPC driver seat source is missing.");

        clone.AddComponent<BMWM4G82AmbientTrafficAppearance>();
        clone.AddComponent<BMWM4G82AmbientTrafficLighting>();
        clone.AddComponent<BMWM4G82AmbientTrafficDriver>();
...
internal sealed class BMWM4G82AmbientTrafficLighting : MonoBehaviour
{
    private void Start()
    {
        var lighting = GetComponent<BMWM4G82LightingController>() ??
                       gameObject.AddComponent<BMWM4G82LightingController>();
        lighting.InitializeForAmbientTraffic(BMWM4G82PrivateDriverSupport.Context);
    }
}

[DefaultExecutionOrder(1001)]
internal sealed class BMWM4G82AmbientTrafficDriver : MonoBehaviour
{
    private void Start()
    {
        var driver = GetComponent<BMWM4G82DriverController>() ??
                     gameObject.AddComponent<BMWM4G82DriverController>();
        driver.InitializeForAmbientTraffic(BMWM4G82PrivateDriverSupport.Context);
    }
}
