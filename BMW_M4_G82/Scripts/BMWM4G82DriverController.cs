...
internal sealed class BMWM4G82DriverController : MonoBehaviour
{
    ...
    private void LateUpdate()
    {
        ...
        catch (Exception ex)
        {
            ...
            {
                lastFailure = reason;
                context?.Logger.Warn($"BMWM4G82 driver vehicle={vehicleRoot.GetInstanceID()} " +
    ...
    private void CreateDriver()
    {
        ...
        if (ambientTraffic)
        {
            ...
            sourceNpc.name = "BMWM4G82_NpcAppearanceSource";
        ...
        driverRoot = new GameObject(ambientTraffic ? "BMWM4G82_SeatedNpcDriver" : "BMWM4G82_SeatedPlayer");
    ...
    private void AlignWithSeat()
    {
        ...
        driverRoot.transform.rotation =
            vehicleRoot.rotation * Quaternion.Euler(BackrestPitchDegrees, 0f, 0f);
    ...
    {
        if (ambientTraffic && !BMWM4G82Diagnostics.NpcDriverDebugEnabled)
            return;
        if (BMWM4G82Diagnostics.DebugEnabled)
            context?.Logger.Info($"BMWM4G82 driver vehicle={vehicleRoot?.GetInstanceID()} ambient={ambientTraffic}: {message}");
    }

    private void LogNpcDriverCreated(int rendererCount)
    {
        if (ambientTraffic && BMWM4G82Diagnostics.DebugEnabled &&
            BMWM4G82Diagnostics.NpcDriverDebugEnabled)
            context?.Logger.Info($"BMWM4G82 NPC driver vehicle={vehicleRoot?.GetInstanceID()} " +
