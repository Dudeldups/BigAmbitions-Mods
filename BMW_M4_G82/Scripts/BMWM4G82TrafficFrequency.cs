...
[AddComponentMenu("")]
internal sealed class BMWM4G82TrafficFrequency : MonoBehaviour
{
    ...
    private void HandleVehicleAdded(int vehicleIndex)
    {
        if (TryGetIdleVehicles(out _, out var trafficVehicles))
        {
            ...
            {
                totalSpawns++;
                var prefab = BMWM4G82PrivateDriverSupport.TrafficPrefab;
    ...
    {
        if (totalSpawns == 0 || !BMWM4G82Diagnostics.DebugEnabled ||
            !BMWM4G82Diagnostics.NpcTrafficDebugEnabled)
            return;
        context?.Logger.Info(
            "BMWM4G82 traffic frequency: " +
        ...
        out List<VehicleComponent>? idle, out TrafficVehicles trafficVehicles)
    {
        ...
        if (!missingFieldReported)
        {
            ...
            context?.Logger.Warn(
                "BMWM4G82 traffic frequency: idle list unavailable; NPC frequency unchanged.");
        }
        return false;
    }
    ...
    private void RebalanceIdleVehicles()
    {
        ...
        if (!firstBalanceLogged && BMWM4G82Diagnostics.DebugEnabled &&
            BMWM4G82Diagnostics.NpcTrafficDebugEnabled)
        {
