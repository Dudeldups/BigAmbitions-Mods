...
internal sealed class Porsche911GT3RSLightingController : MonoBehaviour
{
    ...
    private void InitializeVisuals(GameObject root)
    {
        ...
        if (CountLampOverlays() != 9 || (trafficLights == null && beamCount != 2) || CountBlinkerOverlays() != 4)
    ...
    {
        if (trafficLights != null && !Porsche911GT3RSDiagnostics.NpcLightingDebugEnabled)
            return;
        Porsche911GT3RSDiagnostics.Info(
            context,
            $"Porsche911GT3RS lighting vehicle='{gameObject.name}' " +
            $"instance={GetInstanceID()}: {message}");
    }

    private void LogWarning(string message) =>
        context?.Logger.Warn($"Porsche911GT3RS lighting vehicle='{gameObject.name}' " +
                             $"instance={GetInstanceID()}: {message}");
