...
[AddComponentMenu("")]
internal sealed class Porsche911GT3RSTrafficFrequency : MonoBehaviour
{
    ...
    private void HandleVehicleAdded(int vehicleIndex)
    {
        if (TryGetIdleVehicles(out _, out var trafficVehicles))
        {
            ...
            {
                totalSpawns++;
                var prefab = Porsche911GT3RSPrivateDriverSupport.TrafficPrefab;
    ...
    {
        if (totalSpawns == 0 || !Porsche911GT3RSDiagnostics.DebugEnabled ||
            !Porsche911GT3RSDiagnostics.NpcTrafficDebugEnabled)
            return;
        context?.Logger.Info(
            "Porsche911GT3RS traffic frequency: " +
        ...
        out List<VehicleComponent>? idle, out TrafficVehicles trafficVehicles)
    {
        ...
        if (!missingFieldReported)
        {
            ...
            context?.Logger.Warn(
                "Porsche911GT3RS traffic frequency: idle list unavailable; NPC frequency unchanged.");
        }
        return false;
    }
    ...
    private void RebalanceIdleVehicles()
    {
        ...
        if (!firstBalanceLogged && Porsche911GT3RSDiagnostics.DebugEnabled &&
            Porsche911GT3RSDiagnostics.NpcTrafficDebugEnabled)
        {
