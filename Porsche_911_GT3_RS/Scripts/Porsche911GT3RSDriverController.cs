...
internal sealed class Porsche911GT3RSDriverController : MonoBehaviour
{
    ...
    private void LateUpdate()
    {
        ...
        catch (Exception ex)
        {
            ...
            {
                lastFailure = reason;
                context?.Logger.Warn($"Porsche911GT3RS driver vehicle={vehicleRoot.GetInstanceID()} " +
    ...
    private void CreateDriver()
    {
        ...
        if (ambientTraffic)
        {
            ...
            sourceNpc.name = "Porsche911GT3RS_NpcAppearanceSource";
        ...
        driverRoot = new GameObject(ambientTraffic ? "Porsche911GT3RS_SeatedNpcDriver" : "Porsche911GT3RS_SeatedPlayer");
        ...
        if (ambientTraffic && Porsche911GT3RSDiagnostics.DebugEnabled &&
            Porsche911GT3RSDiagnostics.NpcDriverDebugEnabled)
            context?.Logger.Info($"Porsche911GT3RS NPC driver vehicle={vehicleRoot?.GetInstanceID()} " +
    ...
    {
        if (ambientTraffic && !Porsche911GT3RSDiagnostics.NpcDriverDebugEnabled)
            return;
        Porsche911GT3RSDiagnostics.Info(
            context,
            $"Porsche911GT3RS driver vehicle={vehicleRoot?.GetInstanceID()} ambient={ambientTraffic}: {message}");
    }
