...
internal sealed class Porsche911GT3RSAutoParkingGuard : MonoBehaviour
{
    ...
    internal static Porsche911GT3RSAutoParkingGuard Install(
        GameObject owner, ModContext context, string vehicleTypeName)
    {
        var guard = owner.GetComponent<Porsche911GT3RSAutoParkingGuard>() ??
                    owner.AddComponent<Porsche911GT3RSAutoParkingGuard>();
        guard.context = context;
        guard.vehicleTypeName = vehicleTypeName;
        if (LastAutoParkPositionField == null || HudParkingStateField == null)
            Porsche911GT3RSDiagnostics.AutoParkingWarn(context,
                "Porsche911GT3RS auto-parking: required game parking field is unavailable.");
        var marker = Path.Combine(Application.persistentDataPath,
            "ModsLocal", "Porsche_911_GT3_RS", "Config", "auto-parking-diagnostics.enabled");
        if (File.Exists(marker))
        {
            Porsche911GT3RSDiagnostics.DebugEnabled = true;
            Porsche911GT3RSDiagnostics.AutoParkingDebugEnabled = true;
            context.Logger.Info("Porsche911GT3RS auto-parking diagnostics enabled.");
        }
    ...
    private bool DiagnosticsEnabled =>
        Porsche911GT3RSDiagnostics.DebugEnabled &&
        Porsche911GT3RSDiagnostics.AutoParkingDebugEnabled;
    ...
    private void LateUpdate()
    {
        ...
        if (selectedVehicle != vehicle)
        {
            ...
            if (parkingHelper == null)
                Porsche911GT3RSDiagnostics.AutoParkingWarn(context,
                    $"Porsche911GT3RS auto-parking: vehicle={vehicle.GetInstanceID()} " +
        ...
        if (DiagnosticsEnabled &&
            ...
            Porsche911GT3RSDiagnostics.AutoParkingInfo(context,
                $"Porsche911GT3RS auto-parking refresh vehicle={vehicle.GetInstanceID()} " +
    ...
    private IEnumerator ReconcileAfterExit(VehicleController vehicle)
    {
        ...
        if (DiagnosticsEnabled)
            Porsche911GT3RSDiagnostics.AutoParkingInfo(context,
                $"Porsche911GT3RS auto-parking exit vehicle={vehicle.GetInstanceID()} " +
