...
internal sealed class ChevroletCamaro1967CaliperController : MonoBehaviour
{
    private static readonly string[,] BindingNames =
    {
        { "CamaroFixedCaliperFrontLeft", "CamaroWheelFrontLeft" },
        { "CamaroFixedCaliperFrontRight", "CamaroWheelFrontRight" },
        { "CamaroFixedCaliperRearLeft", "CamaroWheelRearLeft" },
        { "CamaroFixedCaliperRearRight", "CamaroWheelRearRight" },
    };
    ...
    public void Initialize(VehicleController controller, ModContext? modContext)
    {
        ...
        ChevroletCamaro1967Diagnostics.Info(
            context,
            $"ChevroletCamaro1967 steering calipers ready vehicle={controller.GetInstanceID()}, " +
        ...
        foreach (var binding in bindings)
        {
            ...
            if (Mathf.Abs(steeringAngle) > 60f)
            {
                if (!warnedAboutInvalidSteering)
                {
                    ...
                    context?.Logger.Warn(
                        $"ChevroletCamaro1967 steering caliper rejected angle={steeringAngle:0.0}deg " +
