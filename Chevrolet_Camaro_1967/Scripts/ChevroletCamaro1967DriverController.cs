...
internal sealed class ChevroletCamaro1967DriverController : MonoBehaviour
{
    private const string SteeringWheelMarker = "INT_STEERING_WHEEL";
    private const string SittingClipName = "SitDeliveryTruck";
    private const float SeatedScale = 0.94f;
    private const float SeatBackLeanDegrees = 5f;
    private const float HandHalfSpacing = 0.19f;
    private const float HandForwardOffset = -0.08f;
    private const float HandHeightOffset = 0f;
    private const float FootRaise = 0.18f;
    private const float FootForwardOffset = 0.26f;
    // Initial pelvis position relative to the supplied Camaro steering-wheel pivot.
    private static readonly Vector3 SeatOffset = new(0f, -0.31f, -0.32f);
    ...
    private void LateUpdate()
    {
        ...
        catch (Exception ex)
        {
            ...
            {
                lastFailure = reason;
                context?.Logger.Warn($"ChevroletCamaro1967 driver vehicle={vehicleRoot?.GetInstanceID()} " +
    ...
    private void CreateDriver()
    {
        ...
        if (ambientTraffic)
        {
            ...
            sourceNpc.name = "ChevroletCamaro1967_NpcAppearanceSource";
            ...
            appearance.SetRandomAppearance(
                (seed & 1) == 0 ? Gender.Male : Gender.Female,
                25 * 365 + seed % (25 * 365),
                seed);
        ...
        if (ambientTraffic)
        {
            foreach (var child in vehicleRoot!.GetComponentsInChildren<Transform>(true))
                if (child.name == "NpcDriverSeatAnchor")
                    steeringWheel = child;
        }
        else
        {
        ...
        if (steeringWheel == null)
            throw new InvalidOperationException("Camaro steering-wheel seat reference is missing.");
        ...
        driverRoot = new GameObject(
            ambientTraffic ? "ChevroletCamaro1967_SeatedNpcDriver" : "ChevroletCamaro1967_SeatedPlayer");
        ...
        poseGraph = PlayableGraph.Create("ChevroletCamaro1967 seated driver");
        ...
        AlignHandsWithWheel();
        RaiseFeetAndKnees();
        LogInfo($"created seated {(ambientTraffic ? "NPC" : "player")} renderers={rendererCount} " +
                $"suppressedRenderers={suppressedCount} scale={SeatedScale:F2} " +
                $"seatBackLean={SeatBackLeanDegrees:F1}deg clip='{sittingClip.name}' " +
        ...
        if (ambientTraffic && ChevroletCamaro1967Diagnostics.DebugEnabled &&
            ChevroletCamaro1967Diagnostics.NpcTrafficDebugEnabled)
            context?.Logger.Info(
                $"ChevroletCamaro1967 NPC driver vehicle={vehicleRoot?.GetInstanceID()} " +
    ...
    private void AlignWithSeat()
    {
        ...
        driverRoot.transform.rotation =
            vehicleRoot.rotation * Quaternion.Euler(-SeatBackLeanDegrees, 0f, 0f);
    ...
    private void AlignHandsWithWheel()
    {
        if (vehicleRoot == null || steeringWheel == null)
            return;
        var wheelCenter = vehicleRoot.InverseTransformPoint(steeringWheel.position);
    ...
    {
        if (leg == null || vehicleRoot == null)
            return;
        var target = vehicleRoot.InverseTransformPoint(leg.End.position);
        target.y += FootRaise;
        target.z += FootForwardOffset;
        leg.AimAt(vehicleRoot.TransformPoint(target), vehicleRoot.forward);
    }
    ...
    private string VehiclePosition(Transform? target) =>
        target != null && vehicleRoot != null
            ? vehicleRoot.InverseTransformPoint(target.position).ToString("F3")
            : "missing";

    private void LogInfo(string message) =>
        ChevroletCamaro1967Diagnostics.Info(
            context,
            $"ChevroletCamaro1967 driver vehicle={vehicleRoot?.GetInstanceID()}: {message}");
    ...
    {
        // Ambient traffic objects are pooled and toggled repeatedly by Gley.
        // Keep the already-built seated NPC hierarchy alive while the vehicle
        // is inactive; rebuilding ~200 transforms plus meshes/materials on
        // every traffic recycle causes severe allocation spikes.
        if (ambientTraffic)
        {
            occupied = false;
            return;
