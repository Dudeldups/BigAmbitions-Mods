...
internal static class ChevroletCamaro1967PrivateDriverSupport
{
    ...
    private const string AiPrefabCacheKey = "Prefabs/Vehicles/chevroletcamaro1967.prefab";
    private const string AiCarTypeName = "ChevroletCamaro1967PrivateDriver";
    private const int PrivateDriverPoolSize = 2;
    private const float AmbientTrafficVisualLift = 0.12f;
    ...
    {
        var message =
            $"ChevroletCamaro1967: chauffeur appearance color='{colorName ?? "<none>"}', " +
            $"paintApplied={paintApplied}.";
        if (paintApplied)
            ChevroletCamaro1967Diagnostics.Info(context, message);
        else
    ...
    {
        var message =
            $"ChevroletCamaro1967: chauffeur departure paint color='{colorName ?? "<none>"}' " +
            $"paintApplied={paintApplied}.";
        if (paintApplied)
            ChevroletCamaro1967Diagnostics.Info(context, message);
        else
    ...
    private static GameObject? CreateAiPrefab(GameObject playerPrefab)
    {
        ...
        clone.name = "ChevroletCamaro1967PrivateDriver";
        ...
        var requiredVisuals = new[]
        {
            "CamaroVisual",
            "CamaroDamageBody",
            "CamaroWheelFrontLeft",
            "CamaroWheelFrontRight",
            "CamaroWheelRearLeft",
            "CamaroWheelRearRight",
        };
        foreach (var visualName in requiredVisuals)
        {
            ...
            // Player wheel height is correct, but the Gley traffic template sits
            // lower. Compensate only on AI clones.
            visual.transform.localPosition += Vector3.up * AmbientTrafficVisualLift;
            SetLayerRecursively(visual.transform, clone.layer);

            if (string.Equals(visualName, "CamaroVisual", StringComparison.Ordinal))
                PruneAmbientTrafficVisual(visual);
        }

        ChevroletCamaro1967Materials.FixSolidMaterials(clone);

        // Ambient traffic uses a cloned vehicle visual, not the player prefab
        // hierarchy. Preserve the player steering-wheel pose as an explicit
        // seat anchor and lift it by the same NPC-only visual correction.
        Transform? playerSeat = null;
        foreach (var child in playerPrefab.GetComponentsInChildren<Transform>(true))
        {
            if (child.name.IndexOf("INT_STEERING_WHEEL", StringComparison.OrdinalIgnoreCase) < 0)
                continue;
            playerSeat = child;
            break;
        }
        if (playerSeat != null)
        {
            ...
            aiSeat.transform.localPosition =
                ...
                Vector3.up * AmbientTrafficVisualLift;
            aiSeat.transform.localRotation =
                Quaternion.Inverse(playerPrefab.transform.rotation) * playerSeat.rotation;
        }
        else
        {
            Debug.LogWarning("ChevroletCamaro1967 NPC driver seat source is missing.");
        }

        clone.AddComponent<ChevroletCamaro1967AmbientTrafficAppearance>();
        clone.AddComponent<ChevroletCamaro1967AmbientTrafficLighting>();
        clone.AddComponent<ChevroletCamaro1967AmbientTrafficDriver>();
        if (!FitAiBodyColliders(clone, playerPrefab))
        {
            UnityEngine.Object.Destroy(clone);
            return null;
        }
        var appearance = clone.AddComponent<ChevroletCamaro1967PrivateDriverAppearance>();
    ...
    private static void PruneAmbientTrafficVisual(GameObject visual)
    {
        // Ambient traffic never needs the full player-only cabin and engine
        // hierarchy. Remove only clearly named top-level branches from the
        // cloned Camaro visual. This uses the same basic Object.Destroy path
        // already used by the working vehicle mods.
        var candidates = new List<GameObject>();
        foreach (var child in visual.GetComponentsInChildren<Transform>(true))
        {
            if (child == null || child == visual.transform)
                continue;

            var name = child.name;
            if (name.IndexOf("INT_", StringComparison.OrdinalIgnoreCase) >= 0 ||
                name.IndexOf("_mm_int", StringComparison.OrdinalIgnoreCase) >= 0 ||
                name.IndexOf("INTERIOR", StringComparison.OrdinalIgnoreCase) >= 0 ||
                name.IndexOf("LOD_A_ENGINE", StringComparison.OrdinalIgnoreCase) >= 0 ||
                ...
                name.IndexOf("PEDAL", StringComparison.OrdinalIgnoreCase) >= 0 ||
        ...
        foreach (var candidate in candidates)
        {
            ...
            for (var parent = candidate.transform.parent;
                 parent != null && parent != visual.transform;
        ...
        ChevroletCamaro1967Diagnostics.TrafficInfo(
            $"ChevroletCamaro1967 NPC visual prune removedRoots={removedRoots}.");
    }
    ...
    private static bool FitAiBodyColliders(GameObject clone, GameObject playerPrefab)
    {
        ...
        {
            Debug.LogWarning("ChevroletCamaro1967 NPC body collider source is missing.");
            return false;
        }
        ...
        var holder = new GameObject("ChevroletCamaro1967 NpcBodyCollider");
        ...
        if (front <= rear)
        {
            Debug.LogWarning("ChevroletCamaro1967 NPC body colliders have invalid bounds.");
            UnityEngine.Object.Destroy(holder);
            return false;
        }

        FitAiNavigationObstacles(clone, left, right, rear, front);
        ChevroletCamaro1967Diagnostics.TrafficInfo($"ChevroletCamaro1967 NPC body boxes={holder.GetComponents<BoxCollider>().Length} " +
        ...
        GameObject clone, float bodyLeft, float bodyRight, float bodyRear, float bodyFront)
    {
        ...
        foreach (var obstacle in clone.GetComponentsInChildren<NavMeshObstacle>(true))
        {
            ...
            {
                Debug.LogWarning($"ChevroletCamaro1967 NPC obstacle could not be fitted name='{obstacle.name}'.");
                continue;
            }
            ...
            {
                Debug.LogWarning($"ChevroletCamaro1967 NPC obstacle has invalid fitted length name='{obstacle.name}'.");
                continue;
            }
            ...
            obstacle.size = size;
            ChevroletCamaro1967Diagnostics.TrafficInfo($"ChevroletCamaro1967 NPC obstacle name='{obstacle.name}' " +
...
internal sealed class ChevroletCamaro1967AmbientTrafficLighting : MonoBehaviour
{
    private void Start()
    {
        var lighting = GetComponent<ChevroletCamaro1967LightingController>() ??
                       gameObject.AddComponent<ChevroletCamaro1967LightingController>();
        lighting.InitializeForAmbientTraffic(ChevroletCamaro1967PrivateDriverSupport.Context);
    }
}

internal sealed class ChevroletCamaro1967AmbientTrafficDriver : MonoBehaviour
{
    private void Start()
    {
        var driver = GetComponent<ChevroletCamaro1967DriverController>() ??
                     gameObject.AddComponent<ChevroletCamaro1967DriverController>();
        driver.InitializeForAmbientTraffic(ChevroletCamaro1967PrivateDriverSupport.Context);
    }
}

[DefaultExecutionOrder(1001)]
internal sealed class ChevroletCamaro1967AmbientTrafficAppearance : MonoBehaviour
{
    ...
    {
        if (!ChevroletCamaro1967Diagnostics.TrafficEnabled || stateLogs >= 12)
            return;
        ...
        stateLogs++;
        ChevroletCamaro1967Diagnostics.TrafficInfo(
            $"ChevroletCamaro1967 NPC id={GetInstanceID()} moving={moving} " +
    ...
    private IEnumerator ApplyNativeTrafficColor()
    {
        ...
        if (GetComponent<PrivateDriverVehicle>() == null)
        {
            // Gley/CarFeatures already assigns the traffic vehicle a native
            // VehicleColor. Apply that one chosen color once to the Camaro's
            // texture-aware body paint, matching Porsche/Ferrari traffic cars.
            // With the Camaro capped at roughly 1% of ambient spawns this work
            // happens rarely and only during NPC initialization.
            var color = GetComponent<CarFeatures>()?.VehicleColor;
            var applied = false;
            if (color != null)
            {
                var paint = GetComponent<ChevroletCamaro1967PaintController>();
                if (paint == null)
                    paint = gameObject.AddComponent<ChevroletCamaro1967PaintController>();
                paint.InitializeForAmbientTraffic(color.name, color);
                applied = paint.HasAppliedColor;
            }

            if (colorLogs++ < 8)
                ChevroletCamaro1967Diagnostics.TrafficInfo(
                    $"ChevroletCamaro1967 ambient NPC id={GetInstanceID()} " +
                    $"color='{(color != null ? color.name : "<none>")}' applied={applied}.");
            if (color != null && !applied && colorLogs <= 8)
                Debug.LogWarning(
                    $"ChevroletCamaro1967 ambient NPC id={GetInstanceID()} " +
                    $"failed to apply '{color.name}'.");
        }
        initializationCoroutine = null;
    }
}
...
internal sealed class ChevroletCamaro1967PrivateDriverAppearance : MonoBehaviour
{
    ...
    {
        { "CamaroWheelFrontLeft", "FL" },
        { "CamaroWheelFrontRight", "FR" },
        { "CamaroWheelRearLeft", "BL" },
        { "CamaroWheelRearRight", "BR" },
    };
    ...
    internal void BindWheelVisuals()
    {
        ...
        BindCaliper("CamaroFixedCaliperFrontLeft", "CamaroWheelFrontLeft");
        BindCaliper("CamaroFixedCaliperFrontRight", "CamaroWheelFrontRight");
        BindCaliper("CamaroFixedCaliperRearLeft", "CamaroWheelRearLeft");
        BindCaliper("CamaroFixedCaliperRearRight", "CamaroWheelRearRight");
    ...
    private IEnumerator InitializePrivateDriverState()
    {
        for (var frame = 0; frame < InitializationFrameLimit; frame++)
        {
            ...
            if (candidate != null && candidate.vehicleInstance != null)
            {
                ...
                ChevroletCamaro1967PrivateDriverSupport.ReportAppearanceResult(
    ...
    private IEnumerator EnsureDepartureAfterDismissal()
    {
        ...
        if (dismissed && trafficVehicle != null && trafficVehicle.gameObject.activeInHierarchy)
        {
            ...
            if (privateDriver?.vehicleInstance != null)
            {
                ...
                ChevroletCamaro1967PrivateDriverSupport.ReportDeparturePaintResult(
                    colorName,
    ...
    private bool RestoreSavedVehicleColor(bool applySpecializedPaint)
    {
        ...
        if (ChevroletCamaro1967PrivateDriverSupport.TryResolveDriverColor(
                colorName,
