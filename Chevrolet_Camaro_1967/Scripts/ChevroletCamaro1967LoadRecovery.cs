...
internal static class ChevroletCamaro1967LoadRecovery
{
    ...
    internal static bool CompleteInterruptedLoad(ModContext? context)
    {
        ...
        foreach (var vehicle in save.privateDriverVehicleInstances)
            hasCamaro |= vehicle != null && string.Equals(vehicle.vehicleTypeName,
                ChevroletCamaro1967Mod.VehicleTypeName, StringComparison.Ordinal);
        if (!hasCamaro)
            return false;

        attemptedManager = manager;
        var exitRegistered = HasNativeExitCallback(manager);
        ChevroletCamaro1967Diagnostics.LoadRecoveryInfo(context, $"ChevroletCamaro1967: chauffeur save startup check: " +
        ...
        {
            context.Logger.Warn("ChevroletCamaro1967: startup recovery skipped: native initialization state differs from the supported interrupted-Awake path.");
            return false;
        }
        ...
        foreach (var vehicle in save.VehicleInstances)
        {
            ...
            {
                context.Logger.Warn($"ChevroletCamaro1967: startup recovery stopped: player vehicle type still unresolved: '{vehicle?.vehicleTypeName ?? "<null record>"}'.");
                return false;
            }
        }
        foreach (var vehicle in save.privateDriverVehicleInstances)
        {
            if (vehicle == null || vehicle.VehicleType == null)
            {
                context.Logger.Warn($"ChevroletCamaro1967: startup recovery stopped: chauffeur vehicle type still unresolved: '{vehicle?.vehicleTypeName ?? "<null record>"}'.");
                return false;
            }
        }
        ...
        try
        {
            ...
            context.Logger.Warn($"ChevroletCamaro1967: recovered interrupted city startup after vehicle registration; " +
        ...
        catch (Exception exception)
        {
            ...
            context.Logger.Warn($"ChevroletCamaro1967: interrupted-load recovery failed: {failure}");
            return false;
        }
    }
    ...
    internal static void ReportInputState(ModContext? context)
    {
        ...
        if (!recoveredManager.isActiveAndEnabled || !InputHelper.IsInitialized())
            context.Logger.Warn($"ChevroletCamaro1967: recovered city is not ready for input: " +
