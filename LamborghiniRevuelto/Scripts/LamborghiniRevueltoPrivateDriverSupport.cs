...
internal static class LamborghiniRevueltoPrivateDriverSupport
{
    ...
    private static GameObject? CreateAiPrefab(GameObject playerPrefab)
    {
        ...
        if (playerSeat != null)
        {
            ...
            Debug.LogWarning("LamborghiniRevuelto NPC driver seat source is missing.");

        clone.AddComponent<LamborghiniRevueltoAmbientTrafficAppearance>();
        clone.AddComponent<LamborghiniRevueltoAmbientTrafficLighting>();
        clone.AddComponent<LamborghiniRevueltoAmbientTrafficDriver>();
...
internal sealed class LamborghiniRevueltoAmbientTrafficLighting : MonoBehaviour
{
    private void Start()
    {
        var lighting = GetComponent<LamborghiniRevueltoLightingController>() ??
                       gameObject.AddComponent<LamborghiniRevueltoLightingController>();
        lighting.InitializeForAmbientTraffic(LamborghiniRevueltoPrivateDriverSupport.Context);
    }
}

internal sealed class LamborghiniRevueltoAmbientTrafficDriver : MonoBehaviour
{
    private void Start()
    {
        var driver = GetComponent<LamborghiniRevueltoDriverController>() ??
                     gameObject.AddComponent<LamborghiniRevueltoDriverController>();
        driver.InitializeForAmbientTraffic(LamborghiniRevueltoPrivateDriverSupport.Context);
    }
}
