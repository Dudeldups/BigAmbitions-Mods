...
internal sealed class LamborghiniRevueltoAutoParkingGuard : MonoBehaviour
{
    ...
    internal static LamborghiniRevueltoAutoParkingGuard Install(
        GameObject owner, ModContext context, string vehicleTypeName)
    {
        var guard = owner.GetComponent<LamborghiniRevueltoAutoParkingGuard>() ??
                    owner.AddComponent<LamborghiniRevueltoAutoParkingGuard>();
        guard.context = context;
        guard.vehicleTypeName = vehicleTypeName;
        if (LastAutoParkPositionField == null || HudParkingStateField == null)
            LamborghiniRevueltoDiagnostics.AutoParkingWarn(context,
                "LamborghiniRevuelto auto-parking: required game parking field is unavailable.");
        var marker = Path.Combine(Application.persistentDataPath,
            "ModsLocal", "LamborghiniRevuelto", "Config", "auto-parking-diagnostics.enabled");
        if (File.Exists(marker))
        {
            LamborghiniRevueltoDiagnostics.DebugEnabled = true;
            LamborghiniRevueltoDiagnostics.AutoParkingDebugEnabled = true;
            context.Logger.Info("LamborghiniRevuelto auto-parking diagnostics enabled.");
        }
    ...
    private bool DiagnosticsEnabled =>
        LamborghiniRevueltoDiagnostics.DebugEnabled &&
        LamborghiniRevueltoDiagnostics.AutoParkingDebugEnabled;
    ...
    private void LateUpdate()
    {
        ...
        if (selectedVehicle != vehicle)
        {
            ...
            if (parkingHelper == null)
                LamborghiniRevueltoDiagnostics.AutoParkingWarn(context,
                    $"LamborghiniRevuelto auto-parking: vehicle={vehicle.GetInstanceID()} " +
        ...
        if (DiagnosticsEnabled &&
            ...
            LamborghiniRevueltoDiagnostics.AutoParkingInfo(context,
                $"LamborghiniRevuelto auto-parking refresh vehicle={vehicle.GetInstanceID()} " +
    ...
    private IEnumerator ReconcileAfterExit(VehicleController vehicle)
    {
        ...
        if (DiagnosticsEnabled)
            LamborghiniRevueltoDiagnostics.AutoParkingInfo(context,
                $"LamborghiniRevuelto auto-parking exit vehicle={vehicle.GetInstanceID()} " +
