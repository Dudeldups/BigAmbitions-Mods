...
[AddComponentMenu("")]
internal sealed class LamborghiniRevueltoTrafficFrequency : MonoBehaviour
{
    ...
    private void HandleVehicleAdded(int vehicleIndex)
    {
        if (TryGetIdleVehicles(out _, out var trafficVehicles))
        {
            ...
            {
                totalSpawns++;
                var prefab = LamborghiniRevueltoPrivateDriverSupport.TrafficPrefab;
    ...
    {
        if (totalSpawns == 0 || !LamborghiniRevueltoDiagnostics.DebugEnabled ||
            !LamborghiniRevueltoDiagnostics.NpcTrafficDebugEnabled)
            return;
        context?.Logger.Info(
            "LamborghiniRevuelto traffic frequency: " +
        ...
        out List<VehicleComponent>? idle, out TrafficVehicles trafficVehicles)
    {
        ...
        if (!missingFieldReported)
        {
            ...
            context?.Logger.Warn(
                "LamborghiniRevuelto traffic frequency: idle list unavailable; NPC frequency unchanged.");
        }
        return false;
    }
    ...
    private void RebalanceIdleVehicles()
    {
        ...
        if (!firstBalanceLogged && LamborghiniRevueltoDiagnostics.DebugEnabled &&
            LamborghiniRevueltoDiagnostics.NpcTrafficDebugEnabled)
        {
