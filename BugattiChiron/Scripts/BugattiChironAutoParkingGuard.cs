...
internal sealed class BugattiChironAutoParkingGuard : MonoBehaviour
{
    ...
    internal static BugattiChironAutoParkingGuard Install(
        GameObject owner, ModContext context, string vehicleTypeName)
    {
        var guard = owner.GetComponent<BugattiChironAutoParkingGuard>() ??
                    owner.AddComponent<BugattiChironAutoParkingGuard>();
        guard.context = context;
        guard.vehicleTypeName = vehicleTypeName;
        if (LastAutoParkPositionField == null || HudParkingStateField == null)
            BugattiChironDiagnostics.AutoParkingWarn(context,
                "BugattiChiron auto-parking: required game parking field is unavailable.");
        var marker = Path.Combine(Application.persistentDataPath,
            "ModsLocal", "BugattiChiron", "Config", "auto-parking-diagnostics.enabled");
        if (File.Exists(marker))
        {
            BugattiChironDiagnostics.DebugEnabled = true;
            BugattiChironDiagnostics.AutoParkingDebugEnabled = true;
            context.Logger.Info("BugattiChiron auto-parking diagnostics enabled.");
        }
    ...
    private bool DiagnosticsEnabled =>
        ...
        BugattiChironDiagnostics.AutoParkingDebugEnabled;
    ...
    private void LateUpdate()
    {
        ...
        if (selectedVehicle != vehicle)
        {
            ...
            if (parkingHelper == null)
                BugattiChironDiagnostics.AutoParkingWarn(context,
                    $"BugattiChiron auto-parking: vehicle={vehicle.GetInstanceID()} " +
        ...
        if (DiagnosticsEnabled &&
            ...
            BugattiChironDiagnostics.AutoParkingInfo(context,
                $"BugattiChiron auto-parking refresh vehicle={vehicle.GetInstanceID()} " +
    ...
    private IEnumerator ReconcileAfterExit(VehicleController vehicle)
    {
        ...
        if (DiagnosticsEnabled)
            BugattiChironDiagnostics.AutoParkingInfo(context,
                $"BugattiChiron auto-parking exit vehicle={vehicle.GetInstanceID()} " +
