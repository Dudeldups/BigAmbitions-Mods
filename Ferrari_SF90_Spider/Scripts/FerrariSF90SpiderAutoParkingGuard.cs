...
internal sealed class FerrariSF90SpiderAutoParkingGuard : MonoBehaviour
{
    ...
    internal static FerrariSF90SpiderAutoParkingGuard Install(
        GameObject owner, ModContext context, string vehicleTypeName)
    {
        var guard = owner.GetComponent<FerrariSF90SpiderAutoParkingGuard>() ??
                    owner.AddComponent<FerrariSF90SpiderAutoParkingGuard>();
        guard.context = context;
        guard.vehicleTypeName = vehicleTypeName;
        if (LastAutoParkPositionField == null || HudParkingStateField == null)
            FerrariSF90SpiderDiagnostics.AutoParkingWarn(context,
                "FerrariSF90Spider auto-parking: required game parking field is unavailable.");
        var marker = Path.Combine(Application.persistentDataPath,
            "ModsLocal", "Ferrari_SF90_Spider", "Config", "auto-parking-diagnostics.enabled");
        if (File.Exists(marker))
        {
            FerrariSF90SpiderDiagnostics.DebugEnabled = true;
            FerrariSF90SpiderDiagnostics.AutoParkingDebugEnabled = true;
            context.Logger.Info("FerrariSF90Spider auto-parking diagnostics enabled.");
        }
    ...
    private bool DiagnosticsEnabled =>
        FerrariSF90SpiderDiagnostics.DebugEnabled &&
        FerrariSF90SpiderDiagnostics.AutoParkingDebugEnabled;
    ...
    private void LateUpdate()
    {
        ...
        if (selectedVehicle != vehicle)
        {
            ...
            if (parkingHelper == null)
                FerrariSF90SpiderDiagnostics.AutoParkingWarn(context,
                    $"FerrariSF90Spider auto-parking: vehicle={vehicle.GetInstanceID()} " +
        ...
        if (DiagnosticsEnabled &&
            ...
            FerrariSF90SpiderDiagnostics.AutoParkingInfo(context,
                $"FerrariSF90Spider auto-parking refresh vehicle={vehicle.GetInstanceID()} " +
    ...
    private IEnumerator ReconcileAfterExit(VehicleController vehicle)
    {
        ...
        if (DiagnosticsEnabled)
            FerrariSF90SpiderDiagnostics.AutoParkingInfo(context,
                $"FerrariSF90Spider auto-parking exit vehicle={vehicle.GetInstanceID()} " +
