...
internal sealed class FerrariSF90SpiderLightingController : MonoBehaviour
{
    ...
    {
        if (trafficLights != null && !FerrariSF90SpiderDiagnostics.NpcLightingDebugEnabled)
            return;
        FerrariSF90SpiderDiagnostics.Info(
            context,
            $"FerrariSF90Spider lighting vehicle='{gameObject.name}' " +
            $"instance={GetInstanceID()}: {message}");
    }

    private void LogWarning(string message) =>
        context?.Logger.Warn($"FerrariSF90Spider lighting vehicle='{gameObject.name}' " +
                             $"instance={GetInstanceID()}: {message}");
