...
[AddComponentMenu("")]
internal sealed class FerrariSF90SpiderTrafficFrequency : MonoBehaviour
{
    ...
    private void HandleVehicleAdded(int vehicleIndex)
    {
        if (TryGetIdleVehicles(out _, out var trafficVehicles))
        {
            ...
            {
                totalSpawns++;
                var prefab = FerrariSF90SpiderPrivateDriverSupport.TrafficPrefab;
    ...
    {
        if (totalSpawns == 0 || !FerrariSF90SpiderDiagnostics.DebugEnabled ||
            !FerrariSF90SpiderDiagnostics.NpcTrafficDebugEnabled)
            return;
        context?.Logger.Info(
            "FerrariSF90Spider traffic frequency: " +
        ...
        out List<VehicleComponent>? idle, out TrafficVehicles trafficVehicles)
    {
        ...
        if (!missingFieldReported)
        {
            ...
            context?.Logger.Warn(
                "FerrariSF90Spider traffic frequency: idle list unavailable; NPC frequency unchanged.");
        }
        return false;
    }
    ...
    private void RebalanceIdleVehicles()
    {
        ...
        if (!firstBalanceLogged && FerrariSF90SpiderDiagnostics.DebugEnabled &&
            FerrariSF90SpiderDiagnostics.NpcTrafficDebugEnabled)
        {
