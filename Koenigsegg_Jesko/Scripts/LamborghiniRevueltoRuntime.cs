...
public sealed class KoenigseggJeskoRuntime : MonoBehaviour
{
    ...
    private readonly Dictionary<int, long> performanceFirstSeen = new Dictionary<int, long>();
    private readonly HashSet<int> performanceWaitingForSavedInstance = new HashSet<int>();
    private readonly HashSet<int> performanceIncompletePassLogged = new HashSet<int>();
    ...
    private float performanceCaptureStartedAt;
    private float performanceSampleStartedAt;
    private int performanceSampleFrames;
    private double performanceSampleFrameMs;
    private float performanceSampleMaximumMs;
    private int performanceSampleOver33Ms;
    private int performanceSampleOver50Ms;
    private int performanceSampleOver100Ms;
    private int performanceSampleGc0;
        ...
        GameObject playerVehiclePrefab)
    {
        ...
        var trafficFrequency = runtime.GetComponent<KoenigseggJeskoTrafficFrequency>() ??
                               runtime.gameObject.AddComponent<KoenigseggJeskoTrafficFrequency>();
    ...
    public void Shutdown()
    {
        ...
        performanceFirstSeen.Clear();
        performanceWaitingForSavedInstance.Clear();
        performanceIncompletePassLogged.Clear();
    ...
    private void Update()
    {
        if (KoenigseggJeskoDiagnostics.DebugEnabled &&
            KoenigseggJeskoDiagnostics.PerformanceDebugEnabled)
            SampleFramePacing();
        ...
        KoenigseggJeskoDiagnostics.PerformanceInfo(context,
            $"Jesko performance player-vehicle-count changed total={vehicleCount} " +
            $"time={Time.realtimeSinceStartup:0.000}s; scheduling configuration.");
        ScheduleInitialization("player-vehicle-count-changed");
    }

    private void SampleFramePacing()
    {
        const float sampleSeconds = 10f;
        const float maximumSeconds = 600f;
        var now = Time.realtimeSinceStartup;
        if (performanceSampleStartedAt <= 0f)
        {
            performanceCaptureStartedAt = now;
            performanceSampleStartedAt = now;
            performanceSampleGc0 = GC.CollectionCount(0);
        }
        if (now - performanceCaptureStartedAt > maximumSeconds)
            return;

        var frameMs = Time.unscaledDeltaTime * 1000f;
        performanceSampleFrames++;
        performanceSampleFrameMs += frameMs;
        performanceSampleMaximumMs = Mathf.Max(performanceSampleMaximumMs, frameMs);
        if (frameMs > 33.3f) performanceSampleOver33Ms++;
        if (frameMs > 50f) performanceSampleOver50Ms++;
        if (frameMs > 100f) performanceSampleOver100Ms++;
        if (now - performanceSampleStartedAt < sampleSeconds)
            return;

        var vehicles = VehicleHelper.AllPlayerVehicles;
        var visibleJeskoCount = 0;
        if (vehicles != null)
            foreach (var vehicle in vehicles)
                if (IsTargetVehicle(vehicle) && vehicle != null && vehicle.gameObject.activeInHierarchy)
                    visibleJeskoCount++;
        KoenigseggJeskoDiagnostics.PerformanceInfo(context,
            $"Jesko performance frames time={now:0.0}s visiblePlayerJesko={visibleJeskoCount} " +
            $"activeNpcJesko={KoenigseggJeskoAmbientTrafficAppearance.ActiveCount} " +
            $"frames={performanceSampleFrames} avg={performanceSampleFrameMs / performanceSampleFrames:0.0}ms " +
            $"max={performanceSampleMaximumMs:0.0}ms over33={performanceSampleOver33Ms} " +
            $"over50={performanceSampleOver50Ms} over100={performanceSampleOver100Ms} " +
            $"gc0={GC.CollectionCount(0) - performanceSampleGc0}.");
        performanceSampleFrames = 0;
        performanceSampleFrameMs = 0d;
        performanceSampleMaximumMs = 0f;
        performanceSampleOver33Ms = performanceSampleOver50Ms = performanceSampleOver100Ms = 0;
        performanceSampleGc0 = GC.CollectionCount(0);
        // Keep the wall-clock origin for the bounded ten-minute capture.
        performanceSampleStartedAt = now;
    }
    ...
    private void ResetPerformanceFrameSample()
    {
        performanceCaptureStartedAt = performanceSampleStartedAt = 0f;
        performanceSampleFrames = 0;
        performanceSampleFrameMs = 0d;
        performanceSampleMaximumMs = 0f;
        performanceSampleOver33Ms = performanceSampleOver50Ms = performanceSampleOver100Ms = 0;
    ...
    private IEnumerator InitializeForLifecycle(string source)
    {
        ...
        {
            var tracePerformance = KoenigseggJeskoDiagnostics.DebugEnabled &&
                                   KoenigseggJeskoDiagnostics.PerformanceDebugEnabled;
            var passStartedAt = tracePerformance ? System.Diagnostics.Stopwatch.GetTimestamp() : 0L;
            ...
            if (tracePerformance)
            {
                var elapsedMs = KoenigseggJeskoDiagnostics.Milliseconds(
                    passStartedAt, System.Diagnostics.Stopwatch.GetTimestamp());
                if (attempt == 1 || attempt == InitializationRetryCount || elapsedMs >= 25d)
                    KoenigseggJeskoDiagnostics.PerformanceInfo(context,
                        $"Jesko performance lifecycle source='{source}' attempt={attempt} " +
                        $"elapsed={elapsedMs:0.0}ms matched={matchedCount} dealer={dealerReady} " +
                        $"privatePool={privateDriverPoolReady} privateService={privateDriverReady}.");
            }
    ...
    private bool TryConfigureVehicle(VehicleController? vehicle)
    {
        ...
        var instanceId = vehicle.GetInstanceID();
        var firstSeenAt = 0L;
        if (tracePerformance && !configuredVehicleIds.Contains(instanceId) &&
            !performanceFirstSeen.TryGetValue(instanceId, out firstSeenAt))
        {
            firstSeenAt = System.Diagnostics.Stopwatch.GetTimestamp();
            performanceFirstSeen[instanceId] = firstSeenAt;
        }
        ...
        {
            if (tracePerformance && performanceWaitingForSavedInstance.Add(instanceId))
                KoenigseggJeskoDiagnostics.PerformanceInfo(context,
                    $"Jesko performance vehicle={instanceId} waiting for saved vehicleInstance " +
                    $"time={Time.realtimeSinceStartup:0.000}s sleep={sleepConfigured}.");
        ...
        try
        {
            var configuredAt = tracePerformance ? System.Diagnostics.Stopwatch.GetTimestamp() : 0L;
            ...
            var chassisReadyAt = tracePerformance ? System.Diagnostics.Stopwatch.GetTimestamp() : 0L;
            ...
            var materialsReadyAt = tracePerformance ? System.Diagnostics.Stopwatch.GetTimestamp() : 0L;
            ...
            var lightingReadyAt = tracePerformance ? System.Diagnostics.Stopwatch.GetTimestamp() : 0L;
            ...
            var damageReadyAt = tracePerformance ? System.Diagnostics.Stopwatch.GetTimestamp() : 0L;
            ...
            var controllersReadyAt = tracePerformance ? System.Diagnostics.Stopwatch.GetTimestamp() : 0L;

            var configurationComplete = wheelPlacements >= 12 && repairedBodyShell &&
                                        deformableBodyMeshes > 0 && powertrainConfigured && sleepConfigured;
            if (tracePerformance &&
                (configurationComplete || performanceIncompletePassLogged.Add(instanceId)))
                KoenigseggJeskoDiagnostics.PerformanceInfo(context,
                    $"Jesko performance configure vehicle={instanceId} " +
                    $"time={Time.realtimeSinceStartup:0.000}s " +
                    $"firstSeenToReady={KoenigseggJeskoDiagnostics.Milliseconds(firstSeenAt, controllersReadyAt):0.0}ms " +
                    $"thisPass={KoenigseggJeskoDiagnostics.Milliseconds(configuredAt, controllersReadyAt):0.0}ms " +
                    $"chassis={KoenigseggJeskoDiagnostics.Milliseconds(configuredAt, chassisReadyAt):0.0}ms " +
                    $"materialsGlass={KoenigseggJeskoDiagnostics.Milliseconds(chassisReadyAt, materialsReadyAt):0.0}ms " +
                    $"lighting={KoenigseggJeskoDiagnostics.Milliseconds(materialsReadyAt, lightingReadyAt):0.0}ms " +
                    $"damage={KoenigseggJeskoDiagnostics.Milliseconds(lightingReadyAt, damageReadyAt):0.0}ms " +
                    $"controllers={KoenigseggJeskoDiagnostics.Milliseconds(damageReadyAt, controllersReadyAt):0.0}ms " +
                    $"damageMeshes={deformableBodyMeshes} complete={configurationComplete}.");
            ...
            performanceFirstSeen.Remove(instanceId);
            performanceWaitingForSavedInstance.Remove(instanceId);
            performanceIncompletePassLogged.Remove(instanceId);
